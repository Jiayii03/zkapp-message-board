...
export class Message extends SmartContract {
  ...
  @method async publishMessage(message: Field, signerPrivateKey: PrivateKey) {
    ...
    this.user1.requireEquals(this.user1.get());
    this.user2.requireEquals(this.user2.get());
    this.user3.requireEquals(this.user3.get());
    ...
    signerPublicKey
      ...
      .assertEquals(
        true,
        'Unauthorized signer: signerPublicKey does not match any approved public keys'
      );

    // Update on-chain message state
    this.message.set(message);

    // Add precondition for messageHistoryHash to match the on-chain state
    // this.messageHistoryHash.requireEquals(this.messageHistoryHash.get());
